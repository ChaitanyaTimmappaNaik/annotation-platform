...
async def create_batch(
    ...
    for idx, task_row in enumerate(tasks):
        db.execute(text(
            f"INSERT INTO batch_task_sequences (batch_id, task_id, sequence_order, dataset_object_id) "
            f"VALUES ({batch.id}, {task_row[0]}, {idx}, {idx}) "
            f"ON CONFLICT (batch_id, sequence_order) DO NOTHING"
        ))
        # Update task with dataset_object_id
        db.execute(text(
            f"UPDATE tasks SET dataset_object_id = {idx}, batch_sequence = {idx}, "
            f"required_annotators = {batch_data.required_annotators}, iteration = 1 "
    ...
    for user_id in batch_data.user_ids:
        ...
        for slot, task_row in enumerate(tasks):
            db.execute(text(
                f"INSERT INTO task_assignments (task_id, user_id, batch_id, assignment_slot, status) "
...
async def get_batches(
    ...
    for batch in batches:
        ...
        task_count = db.execute(text(
            f"SELECT COUNT(*) FROM batch_task_sequences WHERE batch_id = {batch.id}"
        )).scalar()
        completed = db.execute(text(
            f"SELECT COUNT(*) FROM task_assignments WHERE batch_id = {batch.id} AND status = 'completed'"
        )).scalar()
        result.append({
            "id": batch.id,
            "name": batch.name,
...
async def get_next_task(
    ...
    next_task = db.execute(text(f"""
        ...
        JOIN batch_task_sequences bts ON bts.task_id = ta.task_id AND bts.batch_id = {batch_id}
    ...
    if not next_task:
        return {"completed": True, "message": "All tasks in this batch completed!"}
...
async def get_batch_progress(
    ...
    total = db.execute(text(
        f"SELECT COUNT(*) FROM task_assignments WHERE batch_id = {batch_id} AND user_id = {current_user.id}"
    )).scalar()
    completed = db.execute(text(
        f"SELECT COUNT(*) FROM task_assignments WHERE batch_id = {batch_id} "
        f"AND user_id = {current_user.id} AND status = 'completed'"
    )).scalar()
    return {
        "total": total,
        "completed": completed,
