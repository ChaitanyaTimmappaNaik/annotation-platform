      ...
      <div style={{ display: "flex", flex: 1 }}>
        ...
        <div style={{ flex: 1, background: "#F2F3F3", padding: 24, overflowY: "auto" }}>
          ...
          <div style={{ fontSize: 12, color: "#687078", marginBottom: 16 }}>
            AnnotateHub &gt; Batches &gt;{" "}
            <strong>Consensus Results — Batch {batchId}</strong>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)",
            ...
            {[
              { label: "Total Tasks", value: results.length,
                color: "#0073BB", bg: "#E8F4FD" },
              { label: "Agreed ✅", value: agreed,
                color: "#1D8102", bg: "#d5f5e3" },
              { label: "Needs Review ⚠️", value: needsReview,
                color: "#D13212", bg: "#FDEDEC" },
              { label: "Pending ⏳", value: pending,
                color: "#996300", bg: "#FEF9E7" },
            ].map(card => (
              <div key={card.label} style={{ background: "white",
                border: "1px solid #D5DBDB", borderRadius: 2, padding: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            <button className="aws-btn-normal" onClick={fetchResults}>
              🔄 Refresh
            </button>
            <button className="aws-btn-primary" onClick={handleExport}>
              ⬇️ Export JSON
            </button>
          ...
          <div style={{ background: "white", border: "1px solid #D5DBDB",
            ...
            {loading ? (
              ...
              <div style={{ padding: 48, textAlign: "center", color: "#687078" }}>
                <div style={{ fontSize: 48, marginBottom: 16 }}>⏳</div>
                <p style={{ fontSize: 14, fontWeight: 600 }}>
                  No consensus results yet.
                </p>
                <p style={{ fontSize: 13 }}>
                  Annotators need to complete tasks first.
                  Once all required annotators submit, consensus
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {results.map(r => {
                    ...
                    const agreePct = Math.round((r.agreement_score || 0) * 100);
                    return (
                      <tr key={r.task_id}
                        ...
                        <td style={{ padding: "10px 16px" }}>
                          <div style={{ display: "flex",
                            ...
                            <div style={{ width: 60, background: "#D5DBDB",
                              borderRadius: 4, height: 6 }}>
                              <div style={{
                                width: `${agreePct}%`,
                                background: agreePct >= 70
                                  ? "#1D8102" : "#D13212",
                                borderRadius: 4, height: 6
                              }} />
                            </div>
                        ...
                        <td style={{ padding: "10px 16px" }}>
                          <span style={{ color: "#0073BB",
                            ...
                            onClick={() => setSelected(
                              selected === r.task_id ? null : r.task_id
                            )}>
          ...
          {selected && (() => {
            ...
            return (
              <div style={{ background: "white", border: "1px solid #D5DBDB",
                ...
                {/* 3 Annotations + Consensus */}
                <div style={{ display: "grid",
                  gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12 }}>
                  {[
                    { label: `W1: ${r.annotator_1 || "—"}`,
                      data: r.annotation_1, color: "#0073BB" },
                    { label: `W2: ${r.annotator_2 || "—"}`,
                      data: r.annotation_2, color: "#8E44AD" },
                    { label: `W3: ${r.annotator_3 || "—"}`,
                      data: r.annotation_3, color: "#D13212" },
                    { label: "✅ Consensus",
                      data: r.consensus_result, color: "#1D8102" },
                  ].map(col => (
                    <div key={col.label}
                      style={{ border: `1px solid ${col.color}30`,
                        borderTop: `3px solid ${col.color}`,
                        borderRadius: 2, padding: 12 }}>
                      <div style={{ fontSize: 12, fontWeight: 700,
                        color: col.color, marginBottom: 8 }}>
                        {col.label}
                      </div>
                      {col.data ? (
                        <div>
                          ...
                          {Object.entries(col.data).map(([key, val]) => (
                            <div key={key} style={{ marginBottom: 4,
                              fontSize: 11, borderBottom: "1px solid #f5f5f5",
                              paddingBottom: 4 }}>
                              <span style={{ fontWeight: 700,
                                color: "#687078" }}>{key}: </span>
